...
3. **Smoke Test**:
   ...
   - Run `pnpm smoke` for end-to-end verification.

4. **Performance Budget**:
   - Run `node scripts/benchmark.js` and ensure report generation is under 2s.

5. **Security & Quality**:
   - Verify `pnpm lint` passes.
   - Verify `pnpm typecheck` passes.
   - Verify `pnpm depcheck` passes (no unused dependencies).
   - Check `docs/AUDIT_REPORT.md` is green.
